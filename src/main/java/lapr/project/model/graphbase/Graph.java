...
public class Graph<V, E> implements GraphInterface<V, E> {
    ...
    public Iterable<Edge<V, E>> edges() {
        ...
        ArrayList<Edge<V, E>> edgeList = new ArrayList<>();//stores the edges
        for (V vert : vertices.keySet()) {                 // keyset() - Returns a Set view of the keys contained in this map.
            for (V vert1 : vertices.keySet()) {            // for cycle verifies edges between vert and vert1
                if (getEdge(vert, vert1) != null) {        // if edge between vert and vert1 exists...
                    edgeList.add(getEdge(vert, vert1));    // add edge to edgeList
                }
            }
        }
        return edgeList;
    ...
    public Iterable<Edge<V, E>> incomingEdges(V vert) {
        if (!validVertex(vert)) {                                //vertex validation         
            return null;                                         //returns null if not valid  
        }
        ArrayList<Edge<V, E>> incomingEdges = new ArrayList<>(); //stores incoming edges

        for (V vertex : vertices.keySet()) {                      // keyset() - Returns a Set view of the keys contained in this map.
            if (getEdge(vertex, vert) != null) {                    // getEdge - Returns the edge from vertex to vert(incoming)
                incomingEdges.add(getEdge(vertex, vert));        //add edge to incoming edges
    ...
    public String toString() {
        ...
        if (this.numVert == 0) {
            sb.append("\nRoadNetwork not defined!");
        } else {
            sb.append("RoadNetwork: ").append(this.numVert).append(" intersections ").append(numEdge).append(" roads\n");

            for (Vertex<V, E> vert : vertices.values()) {
                sb.append(vert).append("\n");
                
                if (vert.getAllOutEdges().iterator().hasNext()) {
                    for (Edge<V, E> entry : vert.getAllOutEdges()) {
                        sb.append("Node Orig: ").append(entry.getVOrig()).append("\n");
                        sb.append("Node Dest: ").append(entry.getVDest()).append("\n");
                    }
                }else {
                    sb.append("\n");
                }
